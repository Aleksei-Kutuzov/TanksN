...
from classes import Bullets, Tanks, Blocks, Buttons, Push_Buttons, Fix_Buttons, Kettlebell_Tanks, Compromise_Tanks, \
    Nimble_APC, Gnat_Tanks, Die_Hard_Tanks, set_game, Invulnerability_Bonus, Settings_game

from neiral_network_class import neuralNetwork
...
class Game_Tanks:
    ...
    def init_N(self):
        self.N_tanks_metod_kall("init_neuralNetwork", (267, 64, 5))
        onlyfiles = [f for f in os.listdir('models') if isfile(posixpath.join('models', f))]
        if onlyfiles != []:
            sk = self.num_t // len(onlyfiles)  # делим на равные части все танчики в зависимости от кол-ва моделей
            num_file = 0  # номер считываемой модели
            n_t = 0  # номер танка, для которого загружаем модель
            for i in self.N_tanks:
                i.neural_N.download_model(onlyfiles[num_file])
                if n_t < len(onlyfiles) - 1:
                    num_file += 1
                else:
                    num_file -= 1
        ...
        self.time_start = self.time_start.tm_min
        self.pop = 0
    ...
    def run_game(self):
        ...
        while self.ran:
            ...
            for i in self.N_tanks:
                ...
                print(i.go_to_neural_network(*self.Tank1.x_and_y, *i.x_and_y, i.direction(), *i.return_side_go(),
                                             *self.vision_list, i.size_to_wall(), fire_may()))
                if self.vision_list[(i.x_and_y[0] + 24) // 48 + (i.x_and_y[1] + 24) // 48 * 16] == 5:
                    i.neural_N.loss += 5
                    self.vision_list[(i.x_and_y[0] + 24) // 48 + (i.x_and_y[1] + 24) // 48 * 16] = 0
                if i.return_side_go()[0] == 1:
                    ...
                    if y > y2 and i.sides != 1:  # and lst_num_bl[1]== 0:
                        ...
                        i.neural_N.train([*self.Tank1.x_and_y,
                                          ...
                                          *self.vision_list,
                                          ...
                                          fire_may()],
                                         (0.1, 0.7, 0.1, 0.1, 0))
                    if y < y2 and i.sides != 0:  # and lst_num_bl[1]== 0:
                        #     print(1)
                        i.neural_N.train([*self.Tank1.x_and_y,
                                          *i.x_and_y,
                                          i.direction(),
                                          ...
                                          *self.vision_list,
                                          ...
                                          fire_may()],
                                         (0.7, 0.1, 0.1, 0.1, 0))
                    if x > x2 and i.sides != 3:  # and lst_num_bl[1]== 0:
                        #     print(1)
                        i.neural_N.train([*self.Tank1.x_and_y,
                                          *i.x_and_y,
                                          i.direction(),
                                          ...
                                          *self.vision_list,
                                          ...
                                          fire_may()],
                                         (0.1, 0.1, 0.1, 0.7, 0))
                    if x < x2 and i.sides != 2:  # and lst_num_bl[1]== 0:
                        #     print(1)
                        i.neural_N.train([*self.Tank1.x_and_y,
                                          *i.x_and_y,
                                          i.direction(),
                        ...
                        if i.return_pozitional()[0] in \
                                ...
                                i.return_pozitional()[1] - self.Tank1.return_pozitional()[1] > 0 and \
                                i.direction() == 0 and i.size_to_wall() > i.return_pozitional()[1] - \
                                self.Tank1.return_pozitional()[1] and \
                                isinstance(i.bullet, int):
                            i.neural_N.train([*self.Tank1.x_and_y,
                                              *i.x_and_y,
                                              i.direction(),
                                              *i.return_side_go(),
                        ...
                        if i.return_pozitional()[0] in \
                                range(self.Tank1.return_pozitional()[0], self.Tank1.return_pozitional()[0] + 48) and \
                                i.return_pozitional()[1] - self.Tank1.return_pozitional()[1] < 0 and \
                                i.direction() == 1 and i.size_to_wall() > i.return_pozitional()[1] - \
                                self.Tank1.return_pozitional()[1] and \
                                isinstance(i.bullet, int):
                            i.neural_N.train([*self.Tank1.x_and_y,
                                              *i.x_and_y,
                        ...
                        if i.return_pozitional()[1] in \
                                ...
                                i.direction() == 2 and i.size_to_wall() > i.return_pozitional()[0] - \
                                self.Tank1.return_pozitional()[0] and \
                                isinstance(i.bullet, int):
                            i.neural_N.train([*self.Tank1.x_and_y,
                                              *i.x_and_y,
                                              i.direction(),
                        ...
                        if i.return_pozitional()[1] in \
                                range(self.Tank1.return_pozitional()[1], self.Tank1.return_pozitional()[1] + 48) and \
                                i.return_pozitional()[0] - self.Tank1.return_pozitional()[0] < 0 and \
                                i.direction() == 3 and i.size_to_wall() > i.return_pozitional()[0] - \
                                self.Tank1.return_pozitional()[0] and \
                                isinstance(i.bullet, int):
                            i.neural_N.train([*self.Tank1.x_and_y,
                                              *i.x_and_y,
