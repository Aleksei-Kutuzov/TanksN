...
class Buttons:
    ...
    def draw(self, window):
        if pg.display.get_init():
            ...
            self.x_and_y[0] + self.size_im[0] / 2 - self.font_for_text.size(self.str_text)[0] / 2,
            self.x_and_y[1] + self.size_im[1] / 2 - self.font_for_text.size(" ")[1] / 2))
...
class Fix_Buttons(Buttons):
    def click_or_not_click(self, event):
        if event == pg.MOUSEBUTTONDOWN:
            ...
            if position[0] in range(self.x_and_y[0], self.x_and_y[0] + self.size_im[0]) and position[1] in range(
                    self.x_and_y[1], self.x_and_y[1] + self.size_im[1]):
                self.get_f()
...
class Bullets:
    # нициализация
    def __init__(self, poz, x_and_y, tank, im_b=set_game.bullets[0], speed=10, damage=1, up_h=22, down_h=23, left_h=23,
                 right_h=22):
        self.x_and_y = x_and_y
        self.damage = damage
...
class Tanks:
    ...
    def init_neuralNetwork(self, input, hidden, output):
        self.neural_N = neuralNetwork(input, hidden, output)

    def go_to_neural_network(self, *p):
        self.sides = []
        self.sides = self.neural_N.query(*p)
        print(self.sides)
        self.sides = self.sides.tolist()

        side = self.sides.index(max(self.sides))
    ...
    def fire(self, up_h=22, down_h=23, left_h=23, right_h=22):
        if self.is_live():
            if isinstance(self.bullet, int):
                self.bullet = Bullets(self.im_t, (self.x_and_y[0], self.x_and_y[1]), self, damage=self.damade,
                                      up_h=up_h, down_h=down_h, left_h=left_h, right_h=right_h)
            ...
            if any((self.bullet.return_pozitional()[0] > set_game.screen_dimensions[0],
                ...
                self.bullet = 0
                return False
            elif collision[0] == "tank" and not collision[1] == self and "enemy" in dir(self) and self.enemy == \
                    collision[1]:
                collision[1].health -= self.bullet.damage
                del self.bullet
                self.bullet = 0
    ...
    def give_effect(self):
        if self.img != None:
            for i in set_game.list_tanks_and_blocks:
                if self.x_and_y[0] in range(i.x_and_y[0] - set_game.delta_tanks_spr,
                                            i.x_and_y[0] + set_game.delta_tanks_spr):
...
class Explosion:
    def __init__(self, parent, x_and_y, window: pg.Surface):
        self.window = window
        self.parent = parent
        self.imgs = [pg.transform.scale(i, [set_game.delta_tanks_spr] * 2) for i in
                     [pg.image.load(r"fragments\fragment_8_16.png"), pg.image.load(r"fragments\fragment_8_17.png"),
